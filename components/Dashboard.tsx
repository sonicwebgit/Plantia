        ...
        <a href={`#/plant/${plant.id}`}>
            ...
            <div className="p-4">
                ...
                <div className="mt-2 text-xs text-slate-400 dark:text-slate-500">
                    {t('dashboard.addedOn', { date: new Date(plant.createdAt).toLocaleDateString(i18n.language) })}
                </div>
            </div>
            </Card>
        </a>
  ...
  return (
    <div className="space-y-6">
      <div className="p-6 rounded-xl bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/50 dark:to-teal-900/50 border border-emerald-100 dark:border-emerald-900">
        <h1 className="text-3xl font-bold text-emerald-800 dark:text-emerald-300">{t('dashboard.title')}</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 px-6 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700">
          ...
          <h2 className="mt-4 text-xl font-semibold text-slate-700 dark:text-slate-300">{t('dashboard.noPlants.title')}</h2>
          <p className="mt-1 text-slate-500 dark:text-slate-400">{t('dashboard.noPlants.subtitle')}</p>

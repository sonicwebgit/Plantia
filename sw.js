 ...
 * @returns {Promise<import('./types').Task[]>} A promise that resolves with an array of tasks.
 */
    ...
    try {
        ...
        if (incompleteToday.length > 0) {
            ...
            await sw.registration.showNotification('Plantia Task Reminder', {
                body: `You have ${incompleteToday.length} plant task${incompleteToday.length > 1 ? 's' : ''} to complete today!`,
                icon: 'vite.svg',
                badge: 'vite.svg',
            });
        } else {
